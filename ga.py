            ...
            for x in range(left, right):
                ...
                if change_block > 7: # 20% chance to change a block
                    ...
                    if choice >= 0 and choice < 10: # enemy: 10%
                        ...
                        genome[y][x] = "B"
                    elif choice >= 20 and choice < 25: # coin: 5%
                        genome[y][x] = "o"
                    elif choice >= 25 and choice < 30: # block with shroom: 5%
                        if y <= 12: # so we can actually hit the block
                            genome[y][x] = "M"
                        else:
                            genome[y][x] = "-"
                    elif choice >= 30 and choice < 35: # block with coin: 5%
                        if y <= 12:  # so we can actually hit the block
                            genome[y][x] = "?"
                        else:
                            genome[y][x] = "-"
                    elif choice >= 35 and choice < 40: # solid wall: 5%
                        genome[y][x] = "X"
                    else:
                        genome[y][x] = "-"             # empty: 60%
        return genome
